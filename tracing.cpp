...
{  
    ...
    while(t--){
        ...
        vector<val> v(m);
        for(int i = 0; i < m; i++){
            cin >> v[i].ti >> v[i].x >> v[i].y;
            v[i].x--;
            v[i].y--;
        }
        sort(v.begin(),v.end(),cmp);
        int ans = 0,glob_mi = inf,glob_ma = 0;
        for(int i = 0; i < n; i++){
            if(s[i] == '0') continue;
            bool pres = false;
            for(int l = 0; l <= m+1; l++){
                ...
                if(temp == s){
                    pres = true;
                    glob_mi = min(glob_mi,l);
                    glob_ma = max(glob_ma,l);
                }
            }
            if(pres) ans++;
        }
        cout<<ans<<" "<<glob_mi<<" ";
        if(glob_ma == m+1) cout<<"Infinity";
        else cout<<glob_ma;
        cout<<"\n";
    }
    return 0;
